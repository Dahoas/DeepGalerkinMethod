...
# Save options
saveOutput = True
saveName   = 'HeatEquation_pointmass'
saveFigure = True
figureName = 'HeatEquation_pointmass.png'
...
def sampler(nSim_interior, nSim_terminal):
    ...
    t_init = 0 * np.zeros((nSim_terminal, 1))
    S_init = np.random.uniform(low=x_low, high=x_high, size = nSim_terminal-3)
    S_init = np.reshape(np.concatenate((S_init,np.array([0.0]*3))),(nSim_terminal,1))
...
def loss(model, t_interior, S_interior, t_terminal, S_terminal):
    ...
    target_payoff = tf.cast(tf.equal(S_terminal,0.0),tf.float32)
