...
def sampler_gaussian(nSim_interior, nSim_terminal,mean,variance):
    ...
    S_init = np.random.normal(mean,variance,size = [nSim_terminal,dim])
    
    return t_interior, S_interior, t_init, S_init

def sampler_uniform(nSim_interior, nSim_terminal,l_bound,r_bound):
    ''' Sample time-space points from the function's domain; points are sampled
        uniformly on the interior of the domain, at the initial/terminal time points
        and along the spatial boundary at different time points. 
    ...
    #Change sampling strategy to sample in areas that matter more
    #Should bound domain somehow
    S_init = np.random.uniform(l_bound,r_bound,size = [nSim_terminal,dim])
...
def run_uniform(l,r):
    ...
    for i in range(sampling_stages):
        ...
        t_interior, S_interior, t_terminal, S_terminal = sampler(nSim_interior, nSim_initial,l,r)
    ...
    plot_loss(losses,"uniform" + str(l) + " " + str(r) + "heat_total_loss")
    plot_loss(l1_losses,"uniform" + str(l) + " " + str(r) + "heat_l1_loss")
    plot_loss(l3_losses,"uniform" + str(l) + " " + str(r) + "heat_l3_loss")
